...
func _on_pressed() -> void:
	if boutonActif != null:
		boutonActif = null
		ecran_de_transition._fondu("InterfaceMenu/MenuGodomon/Resume")
	else:
		if pokemon != null:
			boutonActif = self
			
			ecran_de_transition._fondu("InterfaceMenu/MenuGodomon/Resume")
			get_parent().get_node("Resume/TextureRect_Godomon").texture = boutonActif.sprite
			...
			get_parent().get_node("Resume/TextureRect_Godomon").modulate = boutonActif.couleur
